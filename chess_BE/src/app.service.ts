...
export class AppService {
    ...
    async getLearnTextAndImages(){
        ...
        "Ладья (R):  Ходит   только по вертикалям   или  горизонталям. Может  перемещаться на любое количество пустых клеток, пока нет препятствий, мат можно поставить имея лишь одну ладью.",
        "Слон (B):  Ходит по диагоналям. Движется на любое количество пустых клеток по диагонали. Нельзя поставить шах и мат одним слоном и королем, мат можно поставить только двумя слонами.",
